...
def login():
    if current_user.is_authenticated:
        return redirect(url_for('main.show_map'))
    form = LoginForm()
    if form.validate_on_submit():
        user = User.get_by_email(form.email.data)
        if user and user.check_password(form.password.data):
            login_user(user, remember=form.remember.data)
            return redirect(request.args.get('next') or url_for('main.show_map'))
...
@main_bp.route('/classification_rules', methods=['GET', 'POST'])
@login_required
def classification_rules():
    ...
    rule = ClassificationRule.query.first()
    ...
    if form.validate_on_submit():
        if form.reset.data:
            # Réinitialisation = Valeurs par défaut + mode auto
            rule.mode = 'auto'
            rule.file_size_threshold = 500
            rule.avg_r_threshold = 100
            rule.avg_g_threshold = 100
            rule.avg_b_threshold = 100
            rule.contrast_threshold = 30.0
            rule.laplacian_var_threshold = 100.0
            flash("Règles remises en mode automatique par défaut", "info")
        else:
            # Mise à jour manuelle
            rule.mode = 'manual'
            rule.file_size_threshold = form.file_size_threshold.data
            rule.avg_r_threshold = form.avg_r_threshold.data
            rule.avg_g_threshold = form.avg_g_threshold.data
            rule.avg_b_threshold = form.avg_b_threshold.data
            rule.contrast_threshold = form.contrast_threshold.data
            rule.laplacian_var_threshold = form.laplacian_var_threshold.data
            flash("Règles de classification mises à jour", "success")

        db.session.add(rule)
        db.session.commit()
        return redirect(url_for('main.classification_rules'))

    # Pré-remplir le formulaire avec les valeurs existantes
    if rule.mode == 'manual':
        form.file_size_threshold.data = rule.file_size_threshold
        form.avg_r_threshold.data = rule.avg_r_threshold
        form.avg_g_threshold.data = rule.avg_g_threshold
        form.avg_b_threshold.data = rule.avg_b_threshold
        form.contrast_threshold.data = rule.contrast_threshold
        form.laplacian_var_threshold.data = rule.laplacian_var_threshold

    return render_template("classification_rules.html", form=form, rule=rule)
